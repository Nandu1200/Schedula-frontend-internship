...
function formatTime(value: string) {
  ...
  return `${String(displayHour).padStart(
    2,
    "0"
  )}:${String(minutes).padStart(2, "0")} ${suffix}`;
}
...
export default function DoctorProfilePage() {
  ...
  const [startTimeText, setStartTimeText] =
    useState("");

  const [endTimeText, setEndTimeText] =
    useState("");

  const [slotMessage, setSlotMessage] =
    useState("");

  const [slotMessageType, setSlotMessageType] =
      ...
      if (storedDoctor) {
        try {
          ...
          if (storedSlots) {
            ...
            /*
             * Remove only expired available slots.
             *
             * Past booked slots are preserved because
             * they belong to existing appointments/history.
             */
            const validSlots =
              doctorSlots.filter(
                (slot) =>
  ...
  const handleAddSlot = (
    ...
    const startValid =
      normalizeTimeField("start");

    const endValid =
      normalizeTimeField("end");
    ...
    if (
      ...
      setSlotMessage(
        "Please enter a valid date, start time and end time."
      );
  ...
  const handleDeleteDateSlots = (
    ...
    if (
      ...
      setSlotMessage(
        "Booked slots cannot be removed. Cancel those appointments first."
      );
    ...
    return (
      <main className="min-h-screen bg-[#f2faf7] px-4 py-8 sm:px-6">
        <div className="mx-auto mt-12 max-w-xl rounded-3xl border border-emerald-100 bg-white p-8 text-center shadow-[0_18px_50px_rgba(16,185,129,0.08)]">
          <div className="mx-auto grid size-16 place-items-center rounded-2xl bg-emerald-50 text-2xl text-emerald-600">
            S
          </div>
    ...
    <main className="min-h-screen bg-[#f2faf7] text-slate-900">
      <header className="sticky top-0 z-30 border-b border-emerald-100/80 bg-[#f2faf7]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="grid size-11 place-items-center rounded-2xl bg-emerald-600 text-lg font-extrabold text-white shadow-sm transition group-hover:scale-105">
              S
            </div>
      ...
      <div className="mx-auto max-w-6xl px-4 py-7 sm:px-6 lg:px-8 lg:py-10">
        ...
        <section className="mt-6 rounded-[28px] border border-slate-200/80 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.05)]">
          <div className="border-b border-slate-100 px-6 py-5 sm:px-8">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="hidden rounded-xl bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 sm:block">
                Schedula Doctor
              </div>
            </div>
          </div>
          ...
          >
            ...
            {editing && (
              <div className="mt-7 flex flex-col-reverse gap-3 border-t border-slate-100 pt-6 sm:flex-row sm:justify-end">
                ...
                <button
                  type="submit"
                  className="rounded-xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700 hover:shadow-lg"
                >
                  Save Changes
                </button>
              </div>
        ...
        <section className="mt-6 overflow-hidden rounded-[28px] border border-emerald-100/90 bg-white shadow-[0_20px_60px_rgba(16,185,129,0.07)]">
            ...
            <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                  Choose a date and enter the appointment time range. You can type the time directly or use the clock picker.
                </p>
              </div>
          ...
          <form
            ...
            className="border-b border-slate-100 bg-[#fbfefd] p-6 sm:p-8"
          >
            <div className="grid gap-5 lg:grid-cols-[1.1fr_1fr_1fr_auto] lg:items-end">
              ...
              <div>
                ...
                <div className="relative flex items-center rounded-xl border border-slate-200 bg-white transition focus-within:border-emerald-400 focus-within:ring-4 focus-within:ring-emerald-50">
                  ...
                  <button
                    ...
                    className="mr-1.5 grid size-10 shrink-0 place-items-center rounded-lg text-emerald-600 transition hover:bg-emerald-50"
                  ...
                  <input
                    ...
                    className="pointer-events-none absolute right-2 top-1/2 h-1 w-1 opacity-0"
                    tabIndex={-1}
                    aria-hidden="true"
                  />
                </div>
              ...
              <button
                type="submit"
                className="inline-flex min-h-[52px] items-center justify-center gap-2 rounded-xl bg-emerald-600 px-6 py-3.5 text-sm font-bold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-emerald-700 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2"
              >
            ...
            {slotMessage && (
              <div
                className={`mt-5 flex items-start gap-3 rounded-xl border px-4 py-3.5 text-sm font-semibold ${
                  slotMessageType === "success"
          ...
          <div className="p-6 sm:p-8">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Each time range is added manually and can be managed independently.
                </p>
              </div>
            ...
            {groupedSlots.length ===
              ...
              <div className="mt-7 rounded-2xl border border-dashed border-emerald-200 bg-emerald-50/30 px-6 py-12 text-center">
                ...
                <p className="mx-auto mt-1 max-w-md text-sm leading-6 text-slate-500">
                  Select a date, enter your start and end time, then click Add Slot to make a time range bookable.
                </p>
              </div>
            )}
                    ...
                    >
                      <div className="flex flex-col gap-3 border-b border-slate-100 bg-slate-50/70 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-3">
                          <div className="grid size-11 shrink-0 place-items-center rounded-xl bg-emerald-100 text-sm font-bold text-emerald-700">
                            {new Date(
                              `${date}T00:00:00`
                            ).getDate()}
                          </div>
                      ...
                      <div className="grid gap-3 p-5 sm:grid-cols-2 xl:grid-cols-3">
                        {dateSlots.map(
                          (slot) => (
                            <div
                              key={slot.id}
                              className="group flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 transition hover:-translate-y-0.5 hover:border-emerald-200 hover:bg-emerald-50/30"
                            >
                              ...
                              <div className="flex shrink-0 items-center gap-2">
                                ...
                                "available" ? (
                                  <button
                                    ...
                                    className="grid size-9 place-items-center rounded-lg border border-red-100 text-red-500 transition hover:bg-red-50 hover:text-red-600"
                                    aria-label={`Delete ${formatTime(
                                      slot.startTime
                                    )} slot`}
                                  >
                                    ×
                                  ...
                                  <span className="rounded-lg bg-slate-100 px-2.5 py-2 text-[11px] font-bold text-slate-500">
                                    Booked
                                  </span>
                                )}
                              </div>
